...
# just try this with one setting first to see how long it takes
xgb.hyperparameters = expand.grid(
  nrounds = 300,  # number of times residuals are fit
  max_depth = c(3),  # make number of levels in trees
  eta = c(0.01),  # learning rate, how much to weight the models fit to the residuals
  gamma = 1,  # regularization, like L2 regularization -- higher means more general model
  colsample_bytree = 1,  # subsampling columns
  min_child_weight = 1,  # default value, larger means higher bias
  subsample = c(0.4)  # subsampling rows,  around 0.4 is usually best
)
...
class(vi)
vi$importance
# bottom, left, top, right margins -- you may need to adjust these
